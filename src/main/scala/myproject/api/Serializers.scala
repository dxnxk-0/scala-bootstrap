    ...
    def serialize = {
      ...
      user.level match {
        ...
        case UserLevel.Group =>
          common ++ Map("group_id" -> user.groupId, "group_role" -> user.groupRole.toString)
      }
    }
  }

  implicit class ChannelSerialization(channel: Channel) {
    def serialize = Map("channel_id" -> channel.id, "name" -> channel.name)
  }

  implicit class GroupSerialization(group: Group) {
    def serialize = Map("group_id" -> group.id, "name" -> group.name, "channel_id" -> group.channelId)
  }
}
