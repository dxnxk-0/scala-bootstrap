...
object EnvInit {
  ...
  def initEnv() = {
    ...
    val channelAdmin = User(UUID.randomUUID, UserLevel.Channel, "channel-admin", "bob", "the admin", EmailAddress("admin@channel.com"), "Kondor_123", Some(channel.id), None, None, now, now)
    val group = Group(UUID.fromString("e1f4579e-fc83-4741-b533-772830f84cf9"), "demo group", channel.id, now, now)
    val groupAdmin = User(UUID.fromString("a3a7cff3-dd24-4ef9-b408-e074dec2b550"), UserLevel.Group, "group-admin", "John", "Doe1", EmailAddress("admin@group.com"), "Kondor_123", None, Some(group.id), Some(GroupRole.Admin), now, now)
    val groupUser1 = User(UUID.fromString("db9a9f93-eabd-4923-8a59-da5653f0a626"), UserLevel.Group, "group-user-1", "John", "Doe2", EmailAddress("user-1@group.com"), "Kondor_123", None, Some(group.id), None, now, now)
    val groupUser2 = User(UUID.fromString("096f2116-f3b6-4d26-be26-d000d67806b9"), UserLevel.Group, "group-user-2", "John", "Doe3", EmailAddress("user-2@group.com"), "Kondor_123", None, Some(group.id), None, now, now)
